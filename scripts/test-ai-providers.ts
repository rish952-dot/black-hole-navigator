...
const compact = JSON.stringify({ n: 1, g: 0, o: "mesh-connectivity", s: [0.45, 0.6, 0.5, 0.25, 0.4], c: 0.55, e: 4, cost: 1, nov: 0.6, urg: 0.4 });
...
function parseVector(raw: string): Vector {
  ...
  const strategy = Array.isArray(parsed.strategy) ? parsed.strategy.slice(0, 5).map((x) => clamp(x, 0.5)) : [0.5, 0.5, 0.5, 0.5, 0.5];
  while (strategy.length < 5) strategy.push(0.5);
  return { confidence: clamp(parsed.confidence, 0.5), novelty: clamp(parsed.novelty, 0.5), urgency: clamp(parsed.urgency, 0.5), strategy };
}
...
async function testOpenAICompatible(provider: string, apiKey: string | undefined, baseUrl: string, model: string): Promise<Result> {
  ...
  if (!raw) return { provider, status: "FAIL", model, httpStatus: response.status, elapsedMs, error: "empty response" };
  try { return { provider, status: "PASS", model, httpStatus: response.status, elapsedMs, vector: parseVector(raw) }; }
  catch (error) { return { provider, status: "FAIL", model, httpStatus: response.status, elapsedMs, error: `invalid vector JSON: ${error instanceof Error ? error.message : String(error)}` }; }
}
...
async function testClaude(): Promise<Result> {
  ...
  try { return { provider: "claude", status: "PASS", model, httpStatus: response.status, elapsedMs, vector: parseVector(raw) }; }
  catch (error) { return { provider: "claude", status: "FAIL", model, httpStatus: response.status, elapsedMs, error: `invalid vector JSON: ${error instanceof Error ? error.message : String(error)}` }; }
}
...
async function testGemini(): Promise<Result> {
  ...
  try { return { provider: "gemini", status: "PASS", model, httpStatus: response.status, elapsedMs, vector: parseVector(raw) }; }
  catch (error) { return { provider: "gemini", status: "FAIL", model, httpStatus: response.status, elapsedMs, error: `invalid vector JSON: ${error instanceof Error ? error.message : String(error)}` }; }
}
