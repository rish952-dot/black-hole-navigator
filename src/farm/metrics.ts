...
export interface GenerationMetrics {
  generation: number;
  population: number;
  survivalRate: number;       // agents carried over vs terminated
  turnover: number;           // born / population
  avgFitness: number;
  medianFitness: number;
  p90Fitness: number;
  fitnessStdDev: number;
  fitnessImprovement: number; // vs previous generation
  diversity: number;
  netProfit: number;
  roi: number;
  successRate: number;
  throughput: number;         // tasks attempted per agent
...
export function computeGenerationMetrics(
  ...
  const mean = fits.reduce((s, v) => s + v, 0) / Math.max(1, fits.length);
  const sd = Math.sqrt(fits.reduce((s, v) => s + (v - mean) ** 2, 0) / Math.max(1, fits.length));
  const attempted = agents.reduce((s, a) => s + a.stats.tasksAttempted, 0);
  const succeeded = agents.reduce((s, a) => s + a.stats.tasksSucceeded, 0);
...
/** Rolling trend over the last n generations: positive = improving. */
