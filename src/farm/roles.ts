...
/** Advances lifecycle for one generation and reports whether recovery is needed. */
export function advanceLifecycle(state: RoleState, out: CycleOutcome): { state: RoleState; needsRecovery: boolean } {
  const next: RoleState = { ...state, assignedCategories: [...state.assignedCategories] };
  ...
  if (failed || out.netProfit < 0) next.strikes = state.strikes + 1;
  else if (out.attempted > 0) next.strikes = Math.max(0, state.strikes - 1);
